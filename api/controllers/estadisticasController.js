  ...
  try {
    ...
    const result = await conn.execute(`
      SELECT
        ...
        ROUND(AVG(CASE
          WHEN ru.respuesta = p.respuesta_correcta THEN 1
          ELSE 0
        END) * 100, 2) AS promedio_teorico,
        ROUND(AVG(rp.nota), 2) AS promedio_practico,
        COUNT(DISTINCT CASE
          WHEN ru.respuesta = p.respuesta_correcta AND rp.nota >= 70 THEN ex.id_examen
          ELSE NULL
        END) AS aprobados
      FROM CENTRO_EVAL c
      JOIN UBICACION u ON c.no_centro = u.centro_id_centro
      JOIN ESCUELA_AUTOMOV e ON u.escuela_id_escuela = e.no_aut
      LEFT JOIN EXAMEN ex ON ex.registro_id_centro = c.no_centro
